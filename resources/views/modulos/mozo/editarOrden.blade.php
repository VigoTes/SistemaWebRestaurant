            ...
            <div class="text-right">  
              <div  id="guardar">
                  <div class="form-group">
                      ...
                      <a href="{{route('orden.listarSalas')}}" class='btn btn-danger'><i class='fas fa-ban'></i> Cancelar</a>              
                  </div>    
              </div>
            </div>
...
<script type="text/javascript"> 
    ...
    function agregarDetalle3(cod_producto,precioActual,nombre){//agregacion para que se muestren los productos en la tabla (editar orden)
        var fila='<tr class="selected" id="fila'+cont+'">'+
                        '<td style="text-align:center;"><button type="button" class="btn btn-danger btn-xs" onclick="eliminardetalle('+cod_producto+','+cont+');"><i class="fa fa-times" ></i></button></td>'+
                        '<td style="text-align:right;"><input type="number" min="1" onchange="cambioCantidad('+cod_producto+','+cont+')" id="cantidad'+cont+'" name="cantidad[]" value="'+ 1 +'" style="width:80px; text-align:right;"></td>'+
                        '<td><input type="hidden" name="cod_producto[]" value="'+ cod_producto +'" readonly style="width:50px; text-align:right;">'+ nombre +'</td>'+
                        '<td style="text-align:right;"><input type="number" name="pventa[]" value="'+ number_format(precioActual,2) +'" style="width:80px; text-align:right;" readonly></td>'+
                        '<td style="text-align:right;"><input type="number" id="importe'+cont+'" name="importe[]" value="'+ number_format(precioActual,2) +'" style="width:80px; text-align:right;" readonly></td>'+
                    '</tr>';
                    $('#detalles').append(fila);  
 
                subtotal[cont]=1*precioActual;  
    ...
    function cambioCantidad(codigo,index){
        ...
        //alert(detalleventa[pos].codigo+'---'+detalleventa[pos].cantidad);
